<template>
  <div id="DataManagement">
    <div id="Info">
      <h2>Welcome to {{ this.exp_docum.nome_banco_experimental }} database management area</h2>
      <!-- Welcome to  -->
    ...
    <div id="categoria_id">
      <div class="metadata-menu">
        ...
        <div class="metadata-menu-item" id="tutor3">
          <h1>Tutorial</h1>
          <h2> Step 1 : Add categories off behavioral catalog</h2>
          <p>
            To start, add categories used in behavioral catalog.
          </p>

          <h2> Step 2 : Create all metadata of the experiment</h2>
          <p>In sequence, add some metadata categorie your experiment have (ex. animal sex, animal treatment), after add
            all the categorie from that created metada (ex. male and female categories form animal sex metadata categorie
            )
          </p>
          <p> After input all metadata of your experiment click in "Save alterations" button to save the alterations.
            WARNING, after you save you will not more will be able to update the metadata.</p>

          <h2> Step 3 : Add all metadata keys </h2>
            <p>
             Use the meatada edit pannel to add all the keys of experiment metadata created. For example, if was created the animal sex metadata inform the keys: male and female. 

            </p>


          <h2> Step 4: Add all EthoWatcher OS data</h2>
          <p>
            Use the EthoWatcher OS data panel to insert files generated by EthoWatcher OS and organize they using the created metadata information.
          </p>
        ...
        <div class="metadata-menu">
          ...
          <div class="metadata-menu-item" id="categorie">
            ...
            <div v-if="r_exp">
              <Categorie v-bind:experimento="this.exp_docum" @update-exp_docum="handle_update_document" />
            </div>
          </div>
            ...
            <div v-if="r_exp">
              <div class="control-menu">
                ...
                <div class="control-menu-item">
                  <h2>Step 2: Create all metadata of the experiment</h2>
                  Create the metadata of the experiment (e.g., animal sex, treatment, experiment). This information will be use to retrieve information in the query area.
                  <div>
                    <input type="text" placeholder="Input the name of the metadata" v-model="this.t_new_metada" />
                    <input type="button" v-on:click="add_new_metakey()" value="Add new metadata key" />
                  </div>
                  <div class="control-menu-item">
                    <h2>Save your metadata</h2>

                    <input type="button" v-on:click="atualiza_bd()" value="Save alterations" />
    ...
    <div v-if="r_exp">
      <div>
        <ListaMeta v-bind:experimento="this.exp_docum" @update-exp_docum="handle_update_document" />
      </div>
    </div>

    <div id="List_juncoes" v-if="r_exp">
      <ListaJuncoes v-bind:experimento="this.exp_docum" v-on:update_list="force_update()" />
    </div>
